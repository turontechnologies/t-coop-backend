...
/**
 * Builds {@code GET /api/v1/dashboard/summary}. Cards and recentActivity come
 * from real
 * aggregated savings/loan records. The chart has no real hourly-analytics table
 * behind it — it
 * distributes each scope's real totals across the same illustrative hourly
 * shape the frontend's
 * old static mock used, so the numbers are honest sums rather than fully
 * invented ones. Dividends
 * are not a real ledger either; they're shown as 2% of savings, matching the
 * frontend's existing
...
public class DashboardService {
    ...
    private DashboardSummaryDto platformWideSummary() {
        ...
        Map<String, String> cooperativeNames = cooperativeRepository.findAll().stream()
                .collect(Collectors.toMap(Cooperative::getId, Cooperative::getName));

        List<SavingsRecord> savings = savingsRecordRepository
                .findAllByOrderByCreatedAtDesc(PageRequest.of(0, RECENT_ACTIVITY_LIMIT));
        List<LoanRecord> loans = loanRecordRepository
                .findAllByOrderByCreatedAtDesc(PageRequest.of(0, RECENT_ACTIVITY_LIMIT));

        List<RecentActivityDto> recentActivity = mergeRecentActivity(
                savings,
                loans,
    ...
    private DashboardSummaryDto cooperativeSummary(String cooperativeId, String prefix) {
        // "admin" + "member" — same roster CooperativeController.members() (the real Members
        // Directory) counts, so this card always matches what the admin sees on that page.
        long memberCount =
                memberRepository.countByCooperativeIdAndRoleIn(cooperativeId, List.of("admin", "member"));
        ...
        List<SavingsRecord> savings = savingsRecordRepository.findAllByCooperativeIdOrderByCreatedAtDesc(
                cooperativeId, PageRequest.of(0, RECENT_ACTIVITY_LIMIT));
        List<LoanRecord> loans = loanRecordRepository.findAllByCooperativeIdOrderByCreatedAtDesc(
                cooperativeId, PageRequest.of(0, RECENT_ACTIVITY_LIMIT));
    ...
    private DashboardSummaryDto memberSummary(String cooperativeId, String memberId, String prefix) {
        ...
        List<LoanRecord> loans = loanRecordRepository.findAllByMemberIdOrderByCreatedAtDesc(
                memberId, PageRequest.of(0, RECENT_ACTIVITY_LIMIT));
